...
int
	gRingModel
	, nadesUsed[MAXPLAYERS+1]
	, concToUse[MAXPLAYERS+1]
	, nadeId[MAXPLAYERS+1][10]
	, realStart
	, nadeType[MAXPLAYERS+1][10]
	, frameTimer[MAXPLAYERS+1][10];
bool
	holding[MAXPLAYERS+1][10]
	, nadeDelay[MAXPLAYERS+1]
	, buttonDown[MAXPLAYERS+1]
	//, concHelp[MAXPLAYERS]
	, canThrow
	, waitOver;
float
	nadeTime[MAXPLAYERS+1][10]
	, PlayersInRange[MAXPLAYERS+1]
	, holdingArea[3] = { -10000.0, ... }
	, personalTimer[MAXPLAYERS+1]
	, lastTime[MAXPLAYERS+1][10]
	, lastTime2[MAXPLAYERS+1][10]
	, lastOri[MAXPLAYERS+1][10][2][3];
char
	g_classString[16];
Handle
	timeTimer[MAXPLAYERS+1][10];
	...
	if (cvConcEnabled.IntValue == 1) {
		if (IsValidEntity(ent)) {
			...
			if (nadeInfo[0] > -1 && nadeInfo[1] > -1) {
				...
				nadeType[client][number] = nadeId[client][number] = -1;
...
public Action Command_Conc(int client, int args) {
	if (cvConcEnabled.IntValue == 1 && canThrow) {
		...
		if (!IsPlayerAlive(client) || IsFakeClient(client) || IsClientObserver(client) || nadeDelay[client] || !IsClassAllowed(g_classString) || buttonDown[client]) {
...
int MakeNade2(int client, int type = 0) {
	...
	if (number > -1) {
		...
		if (IsValidEntity(nadeId[client][number])) {
			...
			// SOLID_VPHYSICS
			// SetEntProp(nadeId[client][number], Prop_Data, "m_nSolidType", 6);
...
public Action fOnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3]) {
	...
	if (nadeInfo[0] > -1 && nadeInfo[1] > -1) {
		...
		if (IsValidEntity(nadeId[client][number])) {
			float
				now = GetGameTime()
				, delay = cvConcBounceDelay.FloatValue;
	...
	if (nadeInfo[0] > -1 && nadeInfo[1] > -1) {
		...
		if (diffr > 0.01) {
			...
			if (cvNadeSoundMode.IntValue == 0) {
				...
				sEmitSound(-1, (holding[client][number]) ? client : concId, (diffr <= 0.41));
			}
	...
	if (nadeInfo[0] > -1 && nadeInfo[1] > -1) {
		...
		if (nadeType[client][number] == 0) {
			...
			if (cvConcIgnore.IntValue == 1) {
				...
				FindPlayersInRange(center, radius, 0, client, true, nadeId[client][number]);
			}
			int damage = 1;
...
void resetClient(int client) {
	for (int i = 0; i < 10; i++) {
		if (nadeId[client][i] != -1) {
			if (IsValidEntity(nadeId[client][i])) {
...
void classFormat(int class) {
	switch(class) {
		case 1:
			Format(g_classString, sizeof(g_classString), "scout");
		case 2:
			Format(g_classString, sizeof(g_classString), "sniper");
		case 3:
			Format(g_classString, sizeof(g_classString), "soldier");
		case 4:
			Format(g_classString, sizeof(g_classString), "demoman");
		case 5:
			Format(g_classString, sizeof(g_classString), "medic");
		case 6:
			Format(g_classString, sizeof(g_classString), "heavy");
		case 7:
			Format(g_classString, sizeof(g_classString), "pyro");
		case 8:
			Format(g_classString, sizeof(g_classString), "spy");
		case 9:
			Format(g_classString, sizeof(g_classString), "engineer");
	}
}